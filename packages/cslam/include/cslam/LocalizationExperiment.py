...
class LocalizationExperiment(ExperimentAbs):
    ...
    def __callback__(self, msg, _):
        self._lock.acquire()
        # ---
        # if self._graph.number_of_nodes() > self._num_nodes_since_update + self._update_every:
        #     with T2Profiler.profile(f"optimize(N:{self._graph.number_of_nodes()} / E:{self._graph.number_of_edges()})"):
        #         with T2Profiler.profile(f"optimize"):
        #             self.optimize()
        #             self._num_nodes_since_update = self._graph.number_of_nodes()
        ...
        if (not msg.is_static) and (not msg.is_fixed):
            ...
            if not both_movable:
                ...
                self._graph.add_measurement(origin_node_name, target_node_name, origin_time_secs, tf)
            else:
                ...
                if origin_node_name != target_node_name:
                    ...
                    self._graph.add_measurement(origin_node_name, target_node_name, origin_time_secs, TF.from_T(T), information=odom_info_mat)
        # ---
        self._lock.release()
    ...
    def optimize(self):
        with self._lock:
            with T2Profiler.profile("extend-graph"):
                self._extend_graph()
            with T2Profiler.profile("optimize-graph"):
